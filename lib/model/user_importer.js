...
const add_user_interface_schema = Joi.object().required().keys({
    ...
    password     : Joi.string().default(() => randomUUID()),
  });


function add_user(args) {
  let validated_args = add_user_interface_schema.validate(args);
...
function validate_email_to_be_free(args) {
  let validate_args = validate_email_to_be_free_schema.validate(args);
